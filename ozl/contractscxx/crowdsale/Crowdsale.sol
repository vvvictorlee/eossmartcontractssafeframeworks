...
contract Crowdsale {
  ...
  account_name public wallet;

  // How many token units a buyer gets per wei.
  // The rate is the conversion between wei and the smallest and indivisible token unit.
  // So, if you are using a rate of 1 with a DetailedERC20 token with 3 decimals called TOK
  // 1 wei will give you 1 unit, or 0.001 TOK.
  uint256_t public rate;

  // Amount of wei raised
  uint256_t public weiRaised;
   ...
   */
  constructor(uint256_t _rate, account_name _wallet, ERC20 _token) public {
   ...
   */
  function buyTokens(account_name _beneficiary) public payable {

    uint256_t weiAmount = msg.value;
    _preValidatePurchase(_beneficiary, weiAmount);

    // calculate token amount to be created
    uint256_t tokens = _getTokenAmount(weiAmount);
